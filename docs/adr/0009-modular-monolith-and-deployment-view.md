...
- 备份/状态归类口径纠偏（2026-09-09）：修正原"RabbitMQ/Temporal 无状态可重建"——**仅 RabbitMQ 可重放**（依赖 ADR-0001 三 invariant），**Temporal 编排库为持久执行真相**，与业务库同属 pg_dump 备份对象；两库非原子、恢复存在 RPO 近似一致窗口。落点：compose 注释、architecture §5、本 ADR 同步改，权威语义见 docs/ops/backup-restore.md。
