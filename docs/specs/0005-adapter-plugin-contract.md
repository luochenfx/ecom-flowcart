...
| b. 给 `LogisticsTrace` 加逃生口 | 物流响应未映射字段（`logisticsId` / 步骤明细）直通 | **否决（决定性理由 = 闸门，见选项 a 的论据 ①）**：无论落点选哪里，都属 `core-contracts` 字段 / `schemas/**` 结构变更 ⇒ 须另立 Spec 票。落点亦只有两个现实备选（销售侧 `Order` / `OrderSnapshot` 的 `platform_raw` 语义是**销售平台**原文，与此不同源，候选不成立）：**A.** 落到 `PurchaseOrder.platform_raw` ⇒ 与 `createPurchase` 的 raw **争用同一个单节点字段**，两个来源互相覆写（要容纳多端点回传，得先把该字段升级成按端点分组，那是**反转 #55（已合入关闭）的形态决策**）；**B.** 新增 `Tracking` 记录级字段（`Tracking` 是 `PurchaseOrder` 的组件、**非聚合根**：`PurchaseOrder.java:32` = `List<Tracking> tracking`；其 schema `$defs.Tracking` 还被 contract DTO `LogisticsTrace` 复用）⇒ 违背 §3 `:64` 已确立的落点约束（`platform_raw` = **聚合根上的单个 JSONB 节点**，该约束 = #55 已合入形态 + 本决议 D1-c 的一致口径）；即便不论这一条，`fetchLogistics` **一次响应覆盖多条 tracking**，per-`Tracking` 落点还会把同一份原文按 tracking 条数**复制 N 份**（体积 × N）。两条路都属另一张票 |
