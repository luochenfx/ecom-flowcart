...
>
> **时点更新（2026-09-25，#130 落地后）**：上方两处**时点快照已前行**，不回改原文 —— ① `> 来源：` 行的来源票 `#96` 的 `state=OPEN` / `label=needs-triage` 系**起草时点**取值（triage 完成后 label 已变 `ready-for-agent` + `spec-created`，该漂移已在 #128 评论登记）；`#96` 嗣后按**常规生命周期关闭**（AC 4/4 达成）。② `> 状态：` 行的时态「规范态 · **待落地**」**已收敛为「规范态 · 已落地」**：落地票 `#130` 的 PR #131 已合并（`599c301`）。§1 元信息表的同名字段（`命题载体` 的 `needs-triage`、`时态`）**同此口径，不再逐处回改**（承 `0008` §10.1 `deferred` 语义扩展注的同类做法）。本规范对「媒体根唯一权威 = env `FLOWCART_MEDIA_ROOT`」等**实质裁定不受影响**。
