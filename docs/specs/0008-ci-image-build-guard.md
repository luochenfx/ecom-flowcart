...
1. **与既有 `paths-ignore` 的合并校验**——`changes` 闸门叠加后，确认 `image-guard` 仅在白名单路径变更时触发，且既有 `Build & Test (JDK 21)` 覆盖**无变化**（黑名单与 job 级白名单的交互语义须实测，非推理）。**（v2 / #92 扩项，对应 §7 首验 ④）** 同一次实测中须一并确认：**只改 `.github/workflows/ci.yml` 的 PR 能触发 `image-guard`**——这是 Q5(b) 增行的唯一目的，未实测即等于未验收。
   ...
   **实测结果（#94 / PR #97，run `35913855857`，已回填 ⇒ §7 首验 ④ 实测成立、§10 R2 「触达语义」闭合（残余边界见 §10 R2 处置列，不得据此宣称守门自证缺口已闭合））**
...
>
> **（#130 / PR #131 后追加，2026-09-25）** 上表 **R5** 的**改述义务已触发**：`#130` 已把编排层 `app` 服务的媒体根**接线到 env `FLOWCART_MEDIA_ROOT`**（PR #131 合并 / `599c301`）⇒ `/data/media` **现已由代码写入**。故 R5 处置列的条件句按**语义**达成 —— 字面路径「把 `app.media-root` 接线」并未发生（规范 `0011` §4 裁定该配置键保持**非权威声明面、不接线**，接线由 env 承担；二者语义等价）。据此，R5 行前半句「**（该路径当前无代码写）**」**已成为事实过期项**，其判据**改述为「守已接线行为」**；**断言本身无需变更**（与 R5 原文结论一致，`0011` 亦明确守门断言零改动）。

### 10.1 #86 衍生链冻结基线（2026-09-25 登记 · 正式缺口登记处）

> **背景**：本规范所属的 `#86`（CI 增加镜像构建守门）在执行期已衍生 **24 张票 / 四代**（G1 守门落地与断言面 → G2 门禁自身 → G3 守门者的守门 → G4 守门者的守门者的守门）。该扩张机制为「审查发现缺口 → 登记票 → 为缺口票写 Spec → 为 Spec 拆落地票 → 落地票的审查又发现新缺口」，属**元工作递归失控**：第四代已在守「守门者的守门者的守门」。本轮实施**元工作熔断**。
>
> **冻结原则**：本族**不再向下开票**。**冻结 ≠ 已完成** —— 冻结只表示该族不再增生，未落地项以本表登记为准，不靠散落的 issue 维系。
>
> **熔断规则**（规则本体落在 agent 侧 skill，按 `#60` 裁定不入 repo `docs/`）：本族内任一票在实现中发现的缺口 —— 记录在**该票的评论**中，**不向上追溯**（不改祖先票 body / 验收标准）、**不向下开票**。唯一例外通道：缺口属**阻塞性**（不解决会导致 CI 变红 / 功能错误 / 安全漏洞 / 数据丢失）**且**经**人工确认**，方可开**一张**新落地票，且须写明所挂冻结族与确认人。
>
> **冻结锚点**：`#86` 自身保持 OPEN，作为本族唯一入口（其 T1 `#88` / T2 `#89` 均已交付闭环）。

| 票 | 代 | 归属 | 处置 | 内容去向 / 现状 |
|---|---|---|---|---|
| `#88` | G1 | 守门落地 | 已交付 | PR #90 合并；覆盖见 §6 / §7 |
| `#89` | G1 | 断言面 | 已交付 | PR #91 合并；覆盖见 §6 / §7（**注：票面 AC 未逐项勾选**） |
| `#92` | G1 | 断言面 | **冻结关闭**（`deferred`） | 内容已并入本规范 **v2**（§5.4–§5.5 / §6 三路径 union / §7 首验③）+ §10 R1 |
| `#93` | G1 | Spec | **冻结关闭**（`deferred`） | **交付物即本规范 v2**（已落库）⇒ 目标达成，仅补记 |
| `#94` | G1 | 守门自证 | 已交付 | PR #97 合并；§8.1 / §10 R2 触达语义**实测闭合** |
| `#95` | G1 | 断言面 | **冻结关闭**（`deferred`） | 内容保留在 §6 三路径 union（**候选，未落地**）；`blocked by #94` 已解除 |
| `#96` | G1 | **越界登记** | **保留 OPEN（阻塞性）** | **不属本冻结范围**：真实**功能缺陷**（内容链 `media.process` 执行即失败），见 §10 **R3**；按 R3 处置列「另立票」独立推进 |
| `#99` | G2 | 分支保护 | 已关闭 | superseded by `#100` |
| `#100` | G2 | 分支保护 Spec | **冻结关闭**（`deferred`） | 子票 `#101`–`#104` 全闭合、门禁已生效（required = `CI Gate`）；口径残余见 0009 |
| `#101` | G2 | 门禁实现 | 已交付 | PR #108 合并 |
| `#102` | G2 | 规范落库 | 已交付 | `0009-ci-required-status-gate.md` 落库 |
| `#103` | G2 | 守门自证 | 已交付 | PR #111 / #113 / #114 / #120 合并；规范载体 `0010` |
| `#104` | G2 | 门禁切换 | 已执行 | ruleset `main-protection-ci-gates` 的 required = 单条 `CI Gate` |
| `#105` | G2 | 口径同步 | **冻结关闭**（`deferred`） | 纯文档完善度；口径现状以落地时实际报告的 check 集合为准 |
| `#106` | G2 | 验证 | **冻结关闭**（`deferred`） | **目标已实证达成**：PR #126（纯文档）成功合入，其上 `CI Gate` + `Guard Self-Check` 均 success、三条旧检查如期缺席 |
| `#116` | G3 | 自守触发面 | **冻结关闭**（`deferred`） | 内容转 `0010-ci-guard-selfcheck.md` §8.4（**规范态 · 已冻结 · 未实现**） |
| `#117` | G3 | 验证手段 | **冻结关闭**（`deferred`） | 设计已裁定，落 `0011-ci-actionlint-workflow-lint.md`（**未实现**） |
| `#118` | G3 | 文档准确性 | **冻结关闭**（`deferred`） | 涉 `0010` §6.2.1 归类论证 / §9.1 威胁模型边界 |
| `#119` | G3 | 架构级根因 | **冻结关闭**（`deferred`） | 方向登记见 `0010` §9.1 与 §10 R2；检测层已闭合，根因消除未做 |
| `#121` | G4 | 规范落库 | 已交付 | PR #126 合并（`0010` §8.4 进 main） |
| `#122` | G4 | Spec | **冻结关闭**（`deferred`） | `0010` §16 的 A25 follow-up（**未实现**） |
| `#123` | G4 | 实现 | **冻结关闭**（`deferred`） | `0010` §8.4（**规范态 · 未实现**） |
| `#124` | G4 | 取证 | **冻结关闭**（`deferred`） | `0010` §8.4 六段式取证（**未实现**） |
| `#125` | G4 | required 化 | **冻结关闭**（`deferred`） | `0010` 残留 **R-A**（**未闭合**）；须真人 admin |

**冻结后账目（2026-09-25 复算）**：衍生 24 = 已关闭 **23**（既有 9：`88/89/94/99/101/102/103/104/121` + 本轮 14：`92/93/95/100/105/106/116/117/118/119/122/123/124/125`）+ 保留 OPEN **1**（`#96`）。冻结锚点 `#86` 保持 OPEN。

> **（#130 落地后追加，2026-09-25）** 上表 `#96` 行的处置已推进至闭环：该行「保留 OPEN（阻塞性）」所承接的**「另立票」已完成** —— 规范 `0011-media-root-authority-and-wiring.md` 进 main（PR #129 / `383ecb0`），其落地票 `#130` 的 PR #131 合并（`599c301`）⇒ **`#96` 已于 2026-09-25 按常规生命周期关闭**（`state_reason=completed`，AC 4/4 达成；其关闭评论载有 AC 逐条复算与落地证据链）。**冻结族账目随之收敛为：衍生 24 = 已关闭 24 + 保留 OPEN 0**；**冻结锚点 `#86` 保持 OPEN**（本追加不改变该事实）。上表 `#96` 行与上段账目行的原文**保留为 2026-09-25 冻结时点快照，不作回改**。
>
> **口径说明（防重复发现）**：上表 `#96` 行的「阻塞性」与 §10 **R3** 的分级 `non-blocking` **系两个不同维度、非矛盾** —— 前者相对**熔断分级**（不解决会导致功能错误 ⇒ 冻结期不允许 `deferred` 关闭），后者相对 **0008 命题**（该缺陷不影响守门功能，故 §13 将其列为「另立票」）。

> **遗留影响（须与本表一并阅读）**：`0010` 的 §8.4 为**规范态**，其 AC116-1–12、硬约束 HC-1–HC-10、六段式取证与残留 R-A/R-B/R-C **当前无承载票**（`#123`/`#124`/`#125` 已冻结）。该规范的时态声明已同步标注为「**已冻结 · 未实现**」，避免被误读为既成事实 —— 这是本表登记的**已知缺口**之一，非本规范职责内可闭合项。
>
> **`deferred` 标签语义**：在此表示**票级推迟（ticket-level deferral）**，与历史用法（延迟观察**条数**，如 `#44`「deferred 4 条」）不同义，属语义扩展。该标签当前**未收录**于 `docs/agents/triage-labels.md` 的 canonical 集合。
...
- **回滚（v2 / #92，三条，按粒度排列）**：
  ...
  2. **白名单行删除**：删除 `ci.yml` 中 `changes` job → `Filter image-related paths` step → `filters.image` 列表内的**第 6 条** `- '.github/workflows/ci.yml'`（**按名称锚定，勿按行号定位** —— 见本节末「行号易腐」注），退回「改 `ci.yml` 的 PR 不自证」，并按 §10 R2 附注改由 §8.1 人工 / 临时触发验收。
...
> **「行号易腐」注（#94 补救，2026-09-24）**：本规范早期以 `ci.yml` **行号**做锚点（如 `15-25` / `48-56` / `51-52` / `54-56` / `118-125` / `90-96`）。`ci.yml` 经 #88（新增 `changes` + `image-guard` 两个 job）、#89（断言 step）、#94（文件头与 `filters` 前注释 + 白名单第 6 条）多次增补后，**上述行号已整体下移、全部失效**。实测当前位置：`paths-ignore` 20–29、`build` job 三步 53–60、断言 step 125–132、白名单列表 98–103。
>
> **处置口径**：① **操作指引一律按 job / step 名称锚定**（本节「白名单行删除」已改），不再依赖行号；② **§4.2 证据台账中的行号保留为取证时快照**，仅供回溯该条证据的取证语境，**不作为当前定位依据**；③ 后续引用 `ci.yml` **禁止再写行号**，只写名称 —— 行号会随任何一次 `ci.yml` 增补而漂移，是结构性漂移源，不是偶发漏改。
