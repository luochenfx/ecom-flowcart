...
class ProductCatalogContractTest {
    ...
    void jsonToModel_keepsAllFields() throws Exception {
        ...
        assertThat(spu.sourceCategories().getFirst().label()).isEqualTo("数码/影音/音箱");
        ...
        assertThat(catalog.skus().getFirst().sourceSkuId()).isEqualTo("1688-sku-3001");
        assertThat(catalog.skus().getFirst().sourceSpecId()).isEqualTo("b266e0726506185beaf205cbae88530d");
        ...
        Listing listing = catalog.listings().getFirst();
        ...
        assertThat(listing.skuSet().getFirst().enabled()).isTrue();
        assertThat(listing.skuSet().get(1).enabled()).isFalse();
        // catalog Money.amount 为 decimal string
        assertThat(listing.skuSet().getFirst().price().amount()).isEqualTo("79.00");
        assertThat(listing.images().getFirst().platformMediaId()).isEqualTo("tb-media-9001");
        // degraded_steps（#20 内容链降级留痕，specs/0006 §5/§6）：Step id + 原因
        assertThat(listing.degradedSteps()).hasSize(1);
        assertThat(listing.degradedSteps().getFirst().step()).isEqualTo("title.rewrite");
        assertThat(listing.degradedSteps().getFirst().reason()).isNotBlank();
        assertThat(listing.provenance().createdByStep())
                .isEqualTo(io.autocommerce.core.catalog.model.ProvenanceStep.LISTING);

        MediaAsset main = catalog.mediaAssets().getFirst();
